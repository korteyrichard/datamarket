          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-lg font-medium text-gray-900 dark:text-white">Order Pusher API (MTN)</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-300 mt-1">
                    {isApiEnabled ? 'MTN orders are being sent to the external API' : 'MTN orders are not being sent to the external API'}
                  </p>
                </div>
                <button
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-lg font-medium text-gray-900 dark:text-white">CodeCraft API (Telecel, AT, BigTime)</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-300 mt-1">
                    {isCodeCraftApiEnabled ? 'Telecel, AT Data, and BigTime orders are being sent to CodeCraft API' : 'Telecel, AT Data, and BigTime orders are not being sent to CodeCraft API'}
                  </p>
                </div>
                <button
                  onClick={handleCodeCraftApiToggle}
                  disabled={isTogglingCodeCraft}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                    isCodeCraftApiEnabled ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-600'
                  } ${isTogglingCodeCraft ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
